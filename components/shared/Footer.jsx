...
const SocialIcon = ({ href, label, children }) => (
  <a
    href={href}
    aria-label={label}
    className="text-white hover:text-accent-yellow transition-colors p-1 sm:p-1.5 lg:p-2"
  >
    {children}
  </a>
);
  ...
  <div className="flex flex-col items-center select-none" style={{ lineHeight: 0.88 }}>
    <span
      className="font-black uppercase italic text-accent-yellow text-3d-thulla
                 text-[1.5rem] sm:text-[2.2rem] lg:text-[3.2rem]"
    ...
    <span
      className="font-black italic text-[#d9cff5] text-3d-masters
                 text-[1.2rem] sm:text-[1.8rem] lg:text-[2.6rem]"
  ...
  return (
    ...
    >
      <div className="relative flex flex-col items-center pt-7 sm:pt-10 lg:pt-16">
        ...
        {/* Body */}
        <div
          className="relative flex flex-col items-center overflow-hidden flex-shrink-0 w-full
                     h-[185px] sm:h-[260px] lg:h-[340px]"
          style={{
        ...
        >
          ...
          <div
            className="z-10 flex flex-col items-center justify-center flex-shrink-0
                       w-[38px] h-[38px] mt-3 sm:w-[55px] sm:h-[55px] sm:mt-[18px] lg:w-[72px] lg:h-[72px] lg:mt-[26px]"
            style={{ background: "#fcd4b6", borderRadius: "9999px", border: "2px solid #de9f74" }}
          >
            {isLeft ? (
              <>
                <div className="flex gap-1 sm:gap-1.5 mb-0.5">
                  <div className="w-[4px] h-[4px] sm:w-[5px] sm:h-[5px] lg:w-[7px] lg:h-[7px] bg-[#222] rounded-full" />
                  <div className="w-[4px] h-[4px] sm:w-[5px] sm:h-[5px] lg:w-[7px] lg:h-[7px] bg-[#222] rounded-full" />
                </div>
                <div className="w-3 sm:w-4 lg:w-5 border-b-2 border-[#e87474] rounded-full" />
              </>
            ) : (
              <>
                <div className="flex gap-1 sm:gap-1.5 mb-0.5">
                  <div className="w-[7px] h-[2px] sm:w-[10px] sm:h-[3px] lg:w-[13px] lg:h-[4px] bg-[#222] rounded-full" />
                  <div className="w-[7px] h-[2px] sm:w-[10px] sm:h-[3px] lg:w-[13px] lg:h-[4px] bg-[#222] rounded-full" />
                </div>
                <div
                  className="w-4 h-[5px] sm:w-[22px] sm:h-[7px] lg:w-[28px] lg:h-[10px] rounded-full"
                  style={{ background: crownColor }}
                />
              </>
            )}
          </div>
          ...
          <div
            className="z-20 flex items-center justify-center rounded-full bg-white flex-shrink-0
                       w-4 h-4 mt-1 sm:w-6 sm:h-6 sm:mt-2 lg:w-[34px] lg:h-[34px] lg:mt-2.5"
            style={{ border: isLeft ? "2px solid #c00" : "4px solid #000" }}
          >
            <span
              className="text-[0.55rem] sm:text-[0.8rem] lg:text-[1.1rem]"
              style={{ color: suitColor, marginTop: isLeft ? 0 : "-3px" }}
            >
              {suit}
            </span>
          </div>
...
const SOCIALS = [
  {
    label: "Facebook", href: "#",
    svg: <svg viewBox="0 0 24 24" fill="currentColor" className="w-[22px] h-[22px] sm:w-7 sm:h-7 lg:w-[34px] lg:h-[34px]"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" /></svg>,
  },
  {
    label: "WhatsApp", href: "#",
    svg: <svg viewBox="0 0 24 24" fill="currentColor" className="w-[22px] h-[22px] sm:w-7 sm:h-7 lg:w-[34px] lg:h-[34px]"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" /></svg>,
  },
  {
    label: "TikTok", href: "#",
    svg: <svg viewBox="0 0 24 24" fill="currentColor" className="w-[22px] h-[22px] sm:w-7 sm:h-7 lg:w-[34px] lg:h-[34px]"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.77 1.52V6.72a4.85 4.85 0 01-1-.03z" /></svg>,
  },
  {
    label: "Instagram", href: "#",
    svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="w-[22px] h-[22px] sm:w-7 sm:h-7 lg:w-[34px] lg:h-[34px]"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z" /><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" /></svg>,
  },
  {
    label: "Email", href: "#",
    svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="w-[22px] h-[22px] sm:w-7 sm:h-7 lg:w-[34px] lg:h-[34px]"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg>,
  },
];
